...
public class EmailSender {
    ...
    public void send(EmailInfo emailInfo) {
        ...
        this.addBasicDetails(emailInfo, mimeMessage);
        this.addHtmlBody(emailInfo.getTemplate(), emailInfo.getTemplateData(), multipart);
        this.addAttachments(emailInfo.getAttachments(), multipart);
        this.setContent(mimeMessage, multipart);
    ...
    private void addBasicDetails(EmailInfo emailInfo, MimeMessage mimeMessage) {
        try {
            ...
            this.throwEmailSendingException(e, "Error adding data to MIME Message");
        }
    }
    ...
    private void addHtmlBody(String template, Map<String, Object> templateData, MimeMultipart multipart) {
        ...
        try {
            messageHtmlPart.setContent(this.templateEngine.process(template, context), TEXT_HTML_CHARSET_UTF_8);
            multipart.addBodyPart(messageHtmlPart);
        } catch (MessagingException e) {
            this.throwEmailSendingException(e, "Error adding HTML content to MIME Message");
        }
    }
            ...
            attachments.forEach(
                a -> {
                    try {
                        ...
                        this.throwEmailSendingException(e, "Error adding attachments to MIME Message");
                    }
                }
            );
        }
    ...
    private void setContent(MimeMessage mimeMessage, MimeMultipart multipart) {
        try {
            ...
            this.throwEmailSendingException(e, "Error setting content to MIME Message");
        }
    }
