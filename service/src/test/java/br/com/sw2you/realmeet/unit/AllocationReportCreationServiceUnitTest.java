...
class AllocationReportCreationServiceUnitTest extends BaseUnitTest {
    ...
    @BeforeEach
    void setupEach() {
        this.victim = new ReportCreationService(this.reportHandlerResolver, this.reportDispatcherService);

        given(this.reportHandlerResolver.resolveReportHandler(any()))
            .willReturn(
    ...
    void testCreateAllocationReportSuccess() {
        this.victim.createAllocationReport(
                LocalDate.now().minusDays(5),
                LocalDate.now().minusDays(1),
                EMAIL_TO,
                PDF.name()
            );
        verify(this.reportDispatcherService).dispatch(any());
    }
