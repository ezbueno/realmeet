...
@Import(value = ReportApiIntegrationTest.Configuration.class)
class ReportApiIntegrationTest extends BaseIntegrationTest {
    ...
    protected void setupEach() throws MalformedURLException {
        this.setLocalHostBasePath(this.reportApi.getApiClient(), PATH);
    }

    @Test
    void testCreateAllocationReportSuccess() {
        this.persistAllocations(10);

        assertDoesNotThrow(
            () ->
                this.reportApi.createAllocationReport(
                        TEST_CLIENT_API_KEY,
                        EMAIL_TO,
